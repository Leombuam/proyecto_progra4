...
const VIPUserView = () => {
    ...
    const [notifications, setNotifications] = useState("¡Bienvenido! Como usuario VIP, disfruta de acceso exclusivo y beneficios.");
    ...
    useEffect(() => {
        ...
        if (currentUser) {
            setUser({ email: currentUser.email, role: "Usuario VIP" });
        }

        if (location.state?.reservationSuccess) {
            setNotifications("Reserva confirmada. Recuerda presentarte 10 minutos antes del evento.");
        }
    }, [location.state]);
    ...
    return (
        <div>
            ...
            <VIPCarousel auditoriums={auditoriums} />
        </div>
    );
};
...
const Header = ({ onLogout, onProfileClick, onNotificationsClick }) => (
    <header className="header-bar">
        <button className="vip-nav-button" onClick={onProfileClick}>Perfil</button>
        <button className="vip-nav-button" onClick={onNotificationsClick}>Notificaciones</button>
        <button className="vip-nav-button" onClick={onLogout}>Cerrar sesión</button>
    </header>
);
...
const VIPCarousel = ({ auditoriums }) => {
    return (
        <div className="vip-carousel">
            {auditoriums.map((auditorium, index) => (
                <div key={index} className="vip-carousel-item">
                    <img src={auditorium.image} alt={auditorium.name} className="vip-carousel-image" />
                    <div className="vip-carousel-info">
